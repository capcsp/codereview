...
public class CompressString {
  ...
  public String compress(String src) {
    ...
    for (int i = 1; i < src.length(); i++) {
      ...
      if (isSameChar(previousChar, currentChar)) {
        repeatedCharCounter++;
      } 
      else {
  ...
  private boolean isSameChar(char previousChar, char currentChar) {
  ...
  private String compressRecursiveInner(String src, StringBuilder sb, int i, char previousChar,
    ...
    if (thereIsNoMoreWordToCompress == true) {
      ...
      if (isSameChar(src.charAt(i), previousChar)) {
